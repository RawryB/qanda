...
type FormState = "start" | "question" | "completed";
...
export function FormsRunnerClient({
  slug,
  isPreview,
  initialFormInfo,
  initialError,
  ...
  const [logoUrl, setLogoUrl] = useState<string | null>(initialFormInfo?.logoUrl ?? null);
  const [primaryColor, setPrimaryColor] = useState<string>(initialFormInfo?.primaryColor ?? "#0D0D0D");
  const [accentColor, setAccentColor] = useState<string>(initialFormInfo?.accentColor ?? "#4A4744");
  const [transitionColor, setTransitionColor] = useState<string | null>(initialFormInfo?.transitionColor ?? null);
  const [primaryFont, setPrimaryFont] = useState<string>(initialFormInfo?.primaryFont ?? "Syne");
  const [secondaryFont, setSecondaryFont] = useState<string>(initialFormInfo?.secondaryFont ?? "DM Sans");
  ...
  useEffect(() => {
    const href = buildRunnerGoogleFontsHref(resolvedPrimaryFont, resolvedSecondaryFont);
    ...
    return (
      <div style={shellStyle} className={shellClassName}>
        ...
        >
          ...
          <p
            className="type-body-md m-0 whitespace-pre-line"
            style={{ fontFamily: "var(--runner-font-secondary)", color: textMuted }}
          >
            {displayIntroText}
          </p>
          {initialFormInfo && (
    ...
    return (
      <div style={shellStyle} className={shellClassName}>
        ...
        <section className={`relative z-10 flex w-full max-w-[620px] flex-col gap-6 ${questionSectionPaddingClassName}`}>
          <div className="mb-2 flex items-center justify-between">
            ...
            {showQuestionCount && (
              <div
                ...
                style={{ color: backButtonColor, fontFamily: "var(--runner-font-secondary)" }}
              >
          ...
          <h2
            className="type-heading-lg m-0 whitespace-pre-line"
            style={{ fontFamily: "var(--runner-font-primary)", color: textPrimary }}
          >
            {displayTitle}
          </h2>
          {(currentQuestion.renderedHelpText ?? currentQuestion.helpText) && (
            <p
              className="type-body-md -mt-3 m-0"
              style={{ fontFamily: "var(--runner-font-secondary)", color: textMuted }}
            >
              {currentQuestion.renderedHelpText ?? currentQuestion.helpText}
            </p>
          ...
          >
            {renderInput()}
            <div
              className={`flex items-center justify-between gap-3 ${currentQuestion.type === "instruction" ? "flex-row-reverse" : ""}`}
            >
              ...
              <Button
                ...
                className={currentQuestion.type === "instruction" ? "w-full" : ""}
              >
